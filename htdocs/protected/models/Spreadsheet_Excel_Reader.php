...
class Spreadsheet_Excel_Reader {
	...
	function parse_p($row_numbers=false,$col_letters=false,$sheet=0,$table_class='excel') {
		if ($col_letters) {
			$rowspan = $this->rowspan($row,$col,$sheet);
					$colspan = $this->colspan($row,$col,$sheet);
					for($i=0;$i<$rowspan;$i++) {
		...
		$id = 0;
		$cross = array();
		$tmp = array_chunk($arr['val'], 3);
		
		foreach($tmp as $i =>$tab){
			if ($i>0){
				$cross['hanNo'][] = $tmp["$i"]['0'];
				$cross['brand'][] = $tmp["$i"]['1'];
				$cross['partNo'][] = $tmp["$i"]['2'];
				$cross['all'] = $arr['val'];
			}
		}
		return $cross;
	}
	function parse_autoliga($row_numbers=false,$col_letters=false,$sheet=0,$table_class='excel') {
		if ($col_letters) {
			$rowspan = $this->rowspan($row,$col,$sheet);
					$colspan = $this->colspan($row,$col,$sheet);
					for($i=0;$i<$rowspan;$i++) {
		...
		unset ($arr['val']['25']);
		unset ($arr['val']['26']);
		$tmp = array_chunk($arr['val'], 6);

		return $tmp;
	}
	function parse_autoimp($row_numbers=false,$col_letters=false,$sheet=0,$table_class='excel') {
		if ($col_letters) {
			$rowspan = $this->rowspan($row,$col,$sheet);
					$colspan = $this->colspan($row,$col,$sheet);
					for($i=0;$i<$rowspan;$i++) {
		...
		$id = 0;
		$cross = array();
		$tmp = array_chunk($arr['val'], 5);
		return $tmp;
	}
	function parse_impakt($row_numbers=false,$col_letters=false,$sheet=0,$table_class='excel') {
		if ($col_letters) {
			$rowspan = $this->rowspan($row,$col,$sheet);
					$colspan = $this->colspan($row,$col,$sheet);
					for($i=0;$i<$rowspan;$i++) {
		...
		$id = 0;
		$cross = array();
		$tmp = array_chunk($arr['val'], 7);
		return $tmp;
	}
	function parse_alfa($row_numbers=false,$col_letters=false,$sheet=0,$table_class='excel') {
		if ($col_letters) {
			$rowspan = $this->rowspan($row,$col,$sheet);
					$colspan = $this->colspan($row,$col,$sheet);
					for($i=0;$i<$rowspan;$i++) {
	...
	function parse_xls($row_numbers=false,$col_letters=false,$sheet=0,$table_class='excel') {
		return $this->sheets['0']['cells'];
	}
	function parse_bereit($row_numbers=false,$col_letters=false,$sheet=0,$table_class='excel') {
		if ($col_letters) {
			$rowspan = $this->rowspan($row,$col,$sheet);
					$colspan = $this->colspan($row,$col,$sheet);
					for($i=0;$i<$rowspan;$i++) {
		...
		$id = 0;
		$cross = array();
		$tmp = array_chunk($arr['val'], 10);

		return $tmp;
	}
	function parse_autoevro($row_numbers=false,$col_letters=false,$sheet=0,$table_class='excel') {
		if ($col_letters) {
			$rowspan = $this->rowspan($row,$col,$sheet);
					$colspan = $this->colspan($row,$col,$sheet);
					for($i=0;$i<$rowspan;$i++) {
